...
| 動態權重 | 依據近期表現動態調整 |
| 衰減權重 | 近期表現權重較高 |
| **反向權重** | **負權重作為反向信號，排除低效預測號碼** |

### 反向權重策略（Contrarian Negative Weight）

#### 背景
回測分析顯示 Cold-50 演算法的命中率接近隨機期望值（standalone avg_hits ≈ 1.00 vs random 0.73），
且在集成投票中貢獻的信號偏向噪音。將其權重設為負數可將 Cold-50 轉變為「反向信號」——
被 Cold-50 預測的號碼在集成投票中會被扣分，從而避免選中這些低命中率號碼。

#### 原理
```
正常投票: number_votes[num] += weight   (weight > 0 → 增加票數)
反向投票: number_votes[num] += weight   (weight < 0 → 扣減票數)
```
Counter 天然支持負值累加，因此不需修改核心投票邏輯。

#### 回測結果 (2026-02-05)

**測試 A：4 統計演算法，50 期**

| Cold-50 權重 | 平均命中 | Hit 3+ |
|---|---|---|
| 1.19（原始） | 0.78 | 2 |
| 0.0 | **0.84** | 1 |
| -1.0 | 0.80 | 1 |
...
#### 結論
- 負權重效果在 -1.0 飽和，更低的值不再產生變化
- 整體改善幅度微小，但原理正確：消除噪音貢獻
- 設定 Cold-50 = **-1.0** 作為反向信號
- `update_weights_from_backtest()` 已保護負權重不被自動覆蓋

#### 設定
```json
// predict/algorithm_config.json
{
  "ensemble_weights": {
    "Cold-50": -1.0  // 反向信號
  }
...
#### 驗證範圍
- `set_ensemble_weight()` 允許 -3.0 ≤ weight ≤ 5.0
- 自動調參不會覆蓋手動設定的負權重

### 優缺點
- **優點**: 綜合多種方法優勢，減少單一模型偏差；支援反向信號消除噪音
- **缺點**: 計算成本較高（需執行所有模型）
